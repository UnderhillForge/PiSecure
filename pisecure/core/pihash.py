...
class PiHash:
    ...
    def __init__(self, rounds: int = 1, memory_mb: int = 1, npu_enabled: bool = False):
        ...
        Args:
            ...
            memory_mb: Memory usage in MB (default 1MB for fast mining)
    ...
    def _memory_hard_mix(self, hw_hash: bytes, nonce: int) -> bytes:
        ...
        Uses minimal memory (1MB) for fast mining while maintaining hardware-binding
        """
        # Initialize small memory buffer for speed (1MB)
        buffer_size = min(self.memory_bytes, 1024 * 1024)  # Cap at 1MB
        ...
        # Skip expensive mixing loops - just hash the filled buffer
        # This maintains memory-hardness (must allocate buffer) while being fast
