    ...
    try:
        if git:
            ...
            if apply_result['success']:
                ...
                console.print("[yellow]🔄 Restarting PiSecure services...[/yellow]")
                restart_success = _restart_pisecure_services()

                if restart_success:
                    console.print("[green]✅ All services restarted successfully[/green]")
                else:
                    console.print("[yellow]⚠️  Some services may need manual restart[/yellow]")
                    console.print("[dim]Run: sudo systemctl restart pisecure*[/dim]")
            else:
...
def _restart_pisecure_services():
    """Restart all PiSecure services after update"""
    console.print("[yellow]🔄 Restarting PiSecure services...[/yellow]")

    services = [
        'pisecure.service',           # API server
        'pisecure-mining.service',    # Mining service
        'pisecure-dashboard.service', # Dashboard
        'pisecure-relay.service'      # Relay service (if exists)
    ]
    ...
    for service in services:
        try:
            ...
            if result.returncode == 0:  # Service exists and is enabled
                # Restart the service
                ...
                console.print(f"[dim]  ⏭️  {service} not enabled (skipping)[/dim]")
        except subprocess.TimeoutExpired:
            console.print(f"[red]  ⏰ {service} restart timed out[/red]")
    ...
    if restarted:
        console.print(f"[green]✅ Successfully restarted {len(restarted)} service(s)[/green]")

    if failed:
        console.print(f"[red]⚠️  {len(failed)} service(s) failed to restart[/red]")
        console.print("[dim]Manual restart may be required for failed services[/dim]")

    return len(restarted) > 0 and len(failed) == 0
...
def relay_status():
    ...
    try:
        ...
        if is_relay:
            ...
            console.print("   • Active Connections: (feature coming soon)")
            console.print("   • Total Connections: (feature coming soon)")
            console.print("   • Data Relayed: (feature coming soon)")
            console.print("   • Uptime: (feature coming soon)")
        else:
            console.print()
