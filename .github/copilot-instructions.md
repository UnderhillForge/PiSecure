...
⚠️ **IMPORTANT FOR PUBLIC RELEASE**: Hardware verification implementation details have been moved to `/docs/internal/` for security reasons. When assisting users or creating documentation:

- **Public Documentation** (`/docs/PUBLIC_DOCS_INDEX.md`): User-facing guides, API references, getting started
- **Internal Documentation** (`/docs/internal/`): Implementation details, hardware verification internals, debugging notes
- **Implementation Details NOT Public**: VideoCore mailbox specifics, hardware fingerprinting algorithms, PiHash verification logic

When documenting for public release, focus on **what** PiSecure does (secure mining, validation, blockchain), not **how** hardware verification is implemented.
